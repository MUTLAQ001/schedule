...
function parse(text){const lines=text.trim().split('\n').filter(l=>l.trim());const courses=[];const seen=new Set();for(const line of lines){const match=line.match(/\|\s*(\d+)\s*$/);if(match){let name=line.split('|')[0].trim();if(name.includes(' - د.')) name=name.split(' - د.')[0].trim();const key=norm(name)+'|'+match[1];if(seen.has(key))continue;seen.add(key);courses.push({name:name,section:match[1]})}}return courses}
let COURSES=[],FAILS=[];
