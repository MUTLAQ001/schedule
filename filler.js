(function(){
  var payload = {};
  try { payload = JSON.parse(decodeURIComponent(escape(atob(window.QU_FILL || '')))); } catch (e) {}
  var QU_COURSE_LINES = payload.c || '';
  var QU_FREE_CRNS = payload.f || [];
  var MB = !!payload.s;
  function dc(x) { try { return decodeURIComponent(escape(atob(x))); } catch (e) { return ''; } }
  var AC = payload.a || '#8b5cf6';
  var isLightT = !!payload.l;
  function adj(hex, p) { if (!hex || hex.length < 7) return '#7f7f7f'; var r = parseInt(hex.slice(1, 3), 16), g = parseInt(hex.slice(3, 5), 16), b = parseInt(hex.slice(5, 7), 16); p /= 100; r = Math.round(Math.min(255, Math.max(0, r * (1 + p)))); g = Math.round(Math.min(255, Math.max(0, g * (1 + p)))); b = Math.round(Math.min(255, Math.max(0, b * (1 + p)))); return '#' + r.toString(16).padStart(2, '0') + g.toString(16).padStart(2, '0') + b.toString(16).padStart(2, '0'); }
  function rgbOf(hex) { var m = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex); return m ? parseInt(m[1], 16) + ', ' + parseInt(m[2], 16) + ', ' + parseInt(m[3], 16) : '139, 92, 246'; }
  var T = {
    a: AC,
    d: adj(AC, -12),
    d2: adj(AC, -24),
...
const TM=MB?{boot:150,chk:60,appear:900,open:2500,sec:6000,tail:60}:{boot:400,chk:300,appear:0,open:800,sec:500,tail:500};
function sleep(ms){return new Promise(function(r){setTimeout(r,ms)})}
function poll(test,timeout,step){return new Promise(function(res){const t0=Date.now();(function loop(){let ok=false;try{ok=!!test()}catch(e){}if(ok)return res(true);if(Date.now()-t0>=timeout)return res(false);setTimeout(loop,step||40)})()})}
let ALERTS=[],ORIG_ALERT=null;
function hookAlerts(){if(ORIG_ALERT)return;try{ORIG_ALERT=window.alert;window.alert=function(m){ALERTS.push(String(m))}}catch(e){}}
function unhookAlerts(){if(!ORIG_ALERT)return;try{window.alert=ORIG_ALERT}catch(e){}ORIG_ALERT=null}
function dlgVisible(id){const d=document.getElementById(id);return !!(d&&d.getClientRects().length)}
function conflictShown(){return dlgVisible('divsConflictType')}
function closeDialogs(){try{hideDilaogPrimeUI('divsData')}catch(e){}try{hideDilaogPrimeUI('divsConflictType')}catch(e){}}
async function closeDialog(id){for(let k=0;k<8;k++){if(!dlgVisible(id))return true;try{hideDilaogPrimeUI(id)}catch(e){}if(await poll(function(){return !dlgVisible(id)},260,40))return true}const d=document.getElementById(id);if(d&&d.getClientRects().length)d.style.display='none';return !dlgVisible(id)}
function cleanMasks(){if(dlgVisible('divsData')||dlgVisible('divsConflictType'))return;const ms=document.querySelectorAll('.pui-dialog-mask,.ui-widget-overlay');for(const m of ms){if(m.getClientRects().length){try{m.parentNode.removeChild(m)}catch(e){try{m.style.display='none'}catch(e2){}}}}}
function forceHide(id){const d=document.getElementById(id);if(d&&d.getClientRects().length)try{d.style.display='none'}catch(e){}}
function sweepDialogs(){['divsData','divsConflictType'].forEach(function(id){if(dlgVisible(id)){try{hideDilaogPrimeUI(id)}catch(e){}}});cleanMasks()}
function finishCleanup(){sweepDialogs();let n=0;const t=setInterval(function(){n++;sweepDialogs();if(n>=12){clearInterval(t);forceHide('divsData');forceHide('divsConflictType');cleanMasks()}},300)}
function popEl(){return document.getElementById('tableContainer')}
function popSig(){const p=popEl();return p?p.innerHTML:null}
function popHidden(p){return !p||p.getClientRects().length===0}
async function waitPopup(prevSig){if(!MB){await sleep(TM.open);return'fresh'}
const appeared=await poll(function(){return !!popEl()},TM.appear,40);if(!appeared)return'none';
const fresh=await poll(function(){const p=popEl();if(!p)return false;if(!p.querySelector('a[onclick*="changeSction"]'))return false;return prevSig===null||p.innerHTML!==prevSig},TM.open,40);
if(fresh){await sleep(30);return'fresh'}
return popEl()?'stale':'none'}
...
function hasSec(row,sec){return Array.prototype.some.call(row.querySelectorAll('input'),function(i){return String(i.value).trim()===String(sec)})}
...
const rSig=sigOf(row);const prevSig=popSig();link.click();const st=await waitPopup(prevSig);
if(st==='stale'){closeDialogs();undo();return{note:ALERTS[0]||'لم تُفتح قائمة الشعب'}}
const popup=st==='none'?null:popEl();
if(popup){
const links=popup.querySelectorAll('a[onclick*="changeSction"]');const openSig=popup.innerHTML;let found=false;
for(const l of links){if(l.textContent.trim()===c.section){l.click();found=true;await waitApply(row,rSig,openSig);break}}
if(!found){await closeDialog('divsData');undo();return{note:'الشعبة غير مطروحة ضمن شعب هذا المقرر'}}
...
for(const a of links){if(String(a.getAttribute('onclick')||'').replace(/\s+/g,'').indexOf(tail)>=0){link=a;break}}}
