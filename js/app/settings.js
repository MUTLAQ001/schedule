Object.assign(QU_ScheduleApp, {
        _loadSettings() {
          ...
          if (!this.state.userSettings.timeAxisMigrated) {
            this.state.userSettings.timeAxisPosition = 'right';
            this.state.userSettings.timeAxisMigrated = true;
            try { this._saveSettings(); } catch (e) { }
          }
          if (!this.state.userSettings.closedVisibleMigrated) {
            this.state.userSettings.hideClosedCourses = false;
            this.state.userSettings.closedVisibleMigrated = true;
            try { this._saveSettings(); } catch (e) { }
          }
          this._applySettings();
        },
        ...
        _buildSettingsModal() {
          ...
          const loadedCount = this.state.isDemoMode ? 0 : this.state.allCoursesData.length;
        ...
        _attachModalEventListeners() {
          ...
          modal.querySelector('#export-notion-btn')?.addEventListener('click', () => this._handleNotesExport('notion'));
          modal.querySelector('#export-obsidian-btn')?.addEventListener('click', () => this._handleNotesExport('obsidian'));
          ...
          modal.querySelector('#section-edit-btn')?.addEventListener('click', () => this._showSectionEditor());
          ...
          modal.querySelector('#modal-visibility-show-all-btn')?.addEventListener('click', () => this._setAllCoursesVisible(true));
          modal.querySelector('#modal-visibility-hide-all-btn')?.addEventListener('click', () => this._setAllCoursesVisible(false));
        },
        _setAllCoursesVisible(visible) {
          const codes = Object.keys(this.state.groupedCourses);
          if (!codes.length) return;
          if (visible) this.state.hiddenCourseCodes.clear();
          else codes.forEach(code => this.state.hiddenCourseCodes.add(code));
          document.querySelectorAll('.course-visibility-btn').forEach(b => {
            const hidden = this.state.hiddenCourseCodes.has(b.dataset.courseCode);
            b.classList.toggle('hidden', hidden);
            b.classList.toggle('visible', !hidden);
          });
          this._saveSettings();
        ...
        _applySettings() {
          ...
          document.body.classList.toggle('show-evt-times', this.state.userSettings.showEventTimes === true);
          ...
          const mergeToggle = this.dom.settingsModal.querySelector('#merge-courses-toggle');
          if (mergeToggle) { mergeToggle.checked = !!this.state.userSettings.mergeCoursesData; mergeToggle.onchange = e => { this.state.userSettings.mergeCoursesData = e.target.checked; this._saveSettings(); this._showToast(e.target.checked ? 'success' : 'info', e.target.checked ? 'مُفعّل — البيانات القادمة ستُضاف إلى مقرراتك بدل استبدالها.' : 'مُعطّل — البيانات القادمة ستستبدل المقررات الحالية.'); }; }
          const evtTimesToggle = this.dom.settingsModal.querySelector('#event-times-toggle');
          if (evtTimesToggle) { evtTimesToggle.checked = this.state.userSettings.showEventTimes === true; evtTimesToggle.onchange = e => { this.state.userSettings.showEventTimes = e.target.checked; this._saveSettings(); this._applySettings(); }; }
