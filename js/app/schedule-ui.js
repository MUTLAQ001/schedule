...
${statusIndicatorHTML}${favHTML}${this._sectionWatchBtnHTML(section)}<div class="section-btn-number">${this._escapeHTML(section.section)}</div><div class="section-type">${isNoTime ? '<i class="ph ph-laptop"></i>' : ''}${this._escapeHTML(this._typeLabel(section.type))}</div></div>`;
            }).join('');
        ...
        _createMyScheduleHTML(selectedCourses, conflictMap) {
          ...
          const tableRowsHTML = courseGroups.map(g => {
            ...
            return g.items.map((course, idx) => {
              ...
              const closedBadge = this._closedWatchBadgeHTML(course);
          ...
          const gridItemsHTML = selectedCourses.sort((a, b) => a.code.localeCompare(b.code)).map((course, i) => { return `<div class="mobile-schedule-item" style="animation-delay:${i * 30}ms;"><div class="mobile-schedule-header"><div class="mobile-schedule-title"><h3>${this._escapeHTML(course.name)}</h3><span>${this._escapeHTML(course.code)} - شعبة ${this._escapeHTML(course.section)}${this._closedWatchBadgeHTML(course)}</span></div>${conflictMap.has(course.uniqueId) ? '<i class="ph-fill ph-warning mobile-conflict-icon"></i>' : ''}</div><div class="mobile-schedule-details"><div class="mobile-detail-row"><i class="ph ph-clock"></i> <strong>المواعيد:</strong> ${this._escapeHTML(course.time.replace(/<br>/g, ' / ') || 'غير محدد')}</div><div class="mobile-detail-row"><i class="ph ph-user"></i> <strong>المحاضر:</strong> ${this._escapeHTML(course.instructor || 'غير محدد')}</div></div></div>`; }).join('');
...
<div class="ms-part-head"><span class="ms-part-type"><i class="ph ${this._typeIconFor(typeStr)}"></i>${this._escapeHTML(typeStr)}</span><span class="ms-part-sec">شعبة ${this._escapeHTML(c.section)}</span>${this._closedWatchBadgeHTML(c)}${conflicted ? '<span class="conflict-chip"><i class="ph-fill ph-warning"></i>تعارض</span>' : ''}</div>
